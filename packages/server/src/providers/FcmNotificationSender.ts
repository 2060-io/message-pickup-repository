...
export class FcmNotificationSender {
  ...
  constructor(private readonly configService: ConfigService) {
    try {
      ...
      this.admin = null // Set to null to indicate failure
    }
  }
   ...
   * @param {string} token - The device token registered with FCM.
  ...
  public async sendPushNotification(token: string, messageId: string): Promise<SendNotificationResponseDto> {
    try {
      Logger.debug(`[sendPushNotification] Initializing with token: ${token}, messageId: ${messageId}`)

      if (!token || !messageId) {
        Logger.error('[sendPushNotification] Invalid parameters: token or messageId is missing')
      ...
      Logger.debug(`[sendPushNotification] Payload: ${JSON.stringify(messagePayload, null, 2)}`)
